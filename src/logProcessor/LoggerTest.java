...
interface LogProcessor<T extends ILog> {
        List<T> processLogs(List<T> logs);
        }
...
class LogSystem <T extends ILog> {
    ...
    public LogSystem(List<T> elements) {
        logsList = elements;
    }
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = (logs)
                -> logs.stream().filter(log->log.getType().equals("INFO")).collect(Collectors.toList());

        LogProcessor<T> secondLogProcessor = (logs)
                -> logs.stream().filter(log->log.getMessage().length()<100).collect(Collectors.toList());

        LogProcessor<T> thirdLogProcessor = (logs)
                -> logs.stream().sorted(Comparator.comparing(ILog::getTimestamp)).collect(Collectors.toList());
